		...
		*/
		function ArchiveManagerSection(props) {
			...
			const run = async (ids, action) => {
				...
				setNotice({
					kind: "error",
					text: result.failed.length === ids.length ? t("failedDetail", { message: result.failed[0]?.message ?? "" }) : t("failed", { count: String(result.failed.length) })
				});
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											...
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												...
												children: [row.workspaceTitle, row.cwd].filter(Boolean).join(" · ") || String(row.sessionId)
											})]
										}),
		...
		*/
		...
		function rowsEqual(left, right) {
			...
			return left.every((row, index) => {
				...
				return other !== void 0 && row.sessionId === other.sessionId && row.title === other.title && row.cwd === other.cwd && row.updatedAt === other.updatedAt && row.blank === other.blank && row.running === other.running && row.workspaceTitle === other.workspaceTitle;
			});
		}
		/**
